...
| `DOORDASH_WEBHOOK_SECRET` | the exact `Authorization` header value DoorDash sends to the status webhook at `https://admin.<primary>/webhooks/doordash`. DoorDash does **not** sign webhook bodies: when registering the URL in the Developer Portal (Webhooks), pick **Basic Auth** and type the header contents (e.g. `Basic <base64 user:password>`); paste the same string here (a bare `user:password` also works). Verified by `DoorDashWebhookController::authorizationIsValid()` (confirmed against DoorDash's webhook guide 2026-09-02). |
