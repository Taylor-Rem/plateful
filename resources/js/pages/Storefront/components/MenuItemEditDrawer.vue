...
const buildInitial = () => ({
    ...
    default_selection_ids: [
        ...(props.item?.defaultSelectionIds ?? []),
    ] as number[],
});
...
const toggleSingle = (groupId: number, optionId: number | null): void => {
    ...
    form.default_selection_ids = form.default_selection_ids.filter(
    ...
    <Sheet :open="open" @update:open="(v) => emit('update:open', v)">
        <SheetContent class="w-full max-w-xl overflow-y-auto sm:max-w-xl">
            ...
            <form class="space-y-5 px-4 py-4" @submit.prevent="submit">
                ...
                <SheetFooter
                    class="flex-row items-center justify-between gap-2 pt-2"
                >
                    <Button
                        v-if="isEdit && item"
                        type="button"
