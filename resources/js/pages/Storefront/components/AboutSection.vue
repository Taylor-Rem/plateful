...
const hasContent = computed(
    () =>
        Boolean(props.restaurant.aboutBody) ||
        Boolean(props.restaurant.aboutImageUrl),
);

const paragraphs = computed(() =>
    (props.restaurant.aboutBody ?? '')
        .split(/\n+/)
        .map((p) => p.trim())
        .filter(Boolean),
);
...
<template>
    ...
    >
        <button
            v-if="editMode"
            type="button"
            class="absolute top-6 right-6 z-10 inline-flex items-center gap-1 rounded-full bg-card/95 px-2.5 py-1.5 text-xs font-medium text-foreground shadow-sm ring-1 ring-border hover:bg-card"
        ...
        >
            <div
                v-if="
                    restaurant.aboutImageMediumUrl || restaurant.aboutImageUrl
                "
                class="order-1 overflow-hidden rounded-lg bg-muted"
            >
                <img
                    :src="
                        restaurant.aboutImageMediumUrl ??
            ...
            <div
                class="order-2 space-y-4 text-base leading-relaxed text-foreground/90"
            >
