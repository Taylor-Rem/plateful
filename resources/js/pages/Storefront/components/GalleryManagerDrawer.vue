...
const setCaption = (photo: App.Data.RestaurantPhotoData, value: string): void => {
    ...
    <Sheet :open="open" @update:open="(v) => emit('update:open', v)">
        <SheetContent class="w-full max-w-xl overflow-y-auto sm:max-w-xl">
            ...
            <div class="space-y-6 px-4 py-4">
                <!-- Upload -->
                <form class="space-y-3 rounded-md border border-border bg-card p-3" @submit.prevent="uploadPhoto">
                    ...
                    <div class="flex items-start gap-3">
                        <div class="flex size-24 items-center justify-center overflow-hidden rounded-md border border-dashed border-border bg-muted/30">
                            ...
                            <span v-else class="px-2 text-center text-xs text-muted-foreground">Pick an image</span>
                        </div>
                        <div class="flex-1 space-y-2">
                            ...
                            />
                            <Input v-model="uploadForm.caption" maxlength="140" placeholder="Caption (optional)" />
                            <InputError :message="uploadForm.errors.image" />
                            <InputError :message="uploadForm.errors.caption" />
                        </div>
                    </div>
                    <div class="flex justify-end">
                        <Button type="submit" size="sm" :disabled="!uploadForm.image || uploadForm.processing">
                ...
                <div v-if="orderedPhotos.length > 0" class="space-y-3">
                    <p class="text-xs text-muted-foreground">Use the arrows to reorder. Customers see them in this order.</p>
                    <ul class="space-y-2">
                        <li
                        ...
                        >
                            <img
                                :src="photo.imageThumbUrl ?? photo.imageMediumUrl ?? ''"
                                :alt="photo.caption ?? ''"
                            ...
                            <div class="flex-1 space-y-1">
                                <Input
                                    ...
                                    @update:model-value="(v) => setCaption(photo, String(v))"
                ...
                <p v-else class="text-sm text-muted-foreground">No photos yet — add your first above.</p>
            </div>

            <SheetFooter class="flex-row items-center justify-end gap-2 pt-2">
                <Button type="button" variant="outline" @click="close">Done</Button>
            </SheetFooter>
        </SheetContent>
    </Sheet>
</template>
