...
const fullAddress = computed(() => {
    ...
    return [line1, line2, r.postalCode]
...
const hasAnyHours = computed(() =>
    props.restaurant.hoursByDay.some((day) => day.length > 0),
);
</script>
            ...
            <div class="grid gap-8 md:grid-cols-2">
                <div class="space-y-5">
                    <div v-if="fullAddress" class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="font-medium text-foreground">
                                {{ restaurant.street
                                }}<span v-if="restaurant.street2"
                                    >, {{ restaurant.street2 }}</span
                                >
                            </p>
                    ...
                    <div v-if="hasAnyHours">
                        <h3
                            class="mb-2 text-sm font-semibold tracking-wide text-foreground/70 uppercase"
                        >
                            Hours
                        </h3>
                        <dl
                        ...
                        >
                            <div
                                v-for="(windows, dow) in restaurant.hoursByDay"
                                :key="dow"
                                class="flex items-baseline justify-between px-3 py-2 text-sm"
                            >
                                ...
                                <dd class="text-right text-foreground/80">
                                    ...
                                    <span v-else>
                                        ...
                                        >
                                            <span v-if="i > 0">, </span
                                            >{{ formatTime(w.opensAt) }} –
                                            {{ formatTime(w.closesAt) }}
                                        </span>
                                    </span>
                                </dd>
                            </div>
                ...
                <div
                    v-if="mapsEmbedSrc"
                    class="aspect-[4/3] overflow-hidden rounded-lg border border-border bg-muted"
                >
                    <iframe
                        :src="mapsEmbedSrc"
                        class="size-full"
