...
const submit = (): void => {
    if (isEdit.value && props.template) {
        form._method = 'put';
        form.post(`${base.value}/menu/templates/${props.template.id}`, { preserveScroll: true });
    } else {
        form._method = 'post';
...
<template>
    <form class="space-y-6" @submit.prevent="submit">
        <section class="rounded-lg border border-border bg-card p-5">
            <h3 class="text-base font-medium text-foreground">Template details</h3>
            <div class="mt-4 grid gap-4">
                <div class="grid gap-2">
                    <Label for="tpl-name">Name</Label>
                    <Input id="tpl-name" v-model="form.name" required placeholder="e.g. Pizza" />
        ...
        <section class="rounded-lg border border-border bg-card p-5">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-base font-medium text-foreground">Groups</h3>
                    <p class="mt-0.5 text-xs text-muted-foreground">Each group lets the customer pick from a set of options.</p>
                </div>
            ...
            <p v-if="form.groups.length === 0" class="mt-4 text-sm text-muted-foreground">
                No groups yet. Add one to begin (e.g. "Size", "Crust", "Toppings").
            </p>
            ...
            >
                <div class="grid gap-3 sm:grid-cols-12 sm:items-end">
                    <div class="grid gap-1 sm:col-span-4">
                        ...
                        <Input :id="`g-name-${gIndex}`" v-model="group.name" required />
                        <InputError :message="errorAt(`groups.${gIndex}.name`)" />
                    </div>
                    <div class="grid gap-1 sm:col-span-3">
                        <Label :for="`g-min-${gIndex}`">Min selections</Label>
                        <Input :id="`g-min-${gIndex}`" v-model.number="group.min_selections" type="number" min="0" />
                        <InputError :message="errorAt(`groups.${gIndex}.min_selections`)" />
                    </div>
                    <div class="grid gap-1 sm:col-span-3">
                        ...
                        <Input
                            ...
                            placeholder="No max"
                        />
                        <InputError :message="errorAt(`groups.${gIndex}.max_selections`)" />
                    </div>
                    <div class="flex items-center justify-end gap-1 sm:col-span-2">
                        <button
                            type="button"
                ...
                <div class="mt-4 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-foreground">Options</span>
                        <Button type="button" variant="outline" size="sm" @click="addOption(gIndex)">
                            <Plus class="size-4" /> Add option
                        </Button>
                    </div>

                    <p v-if="group.options.length === 0" class="text-xs text-muted-foreground">
                    ...
                    >
                        <div class="grid gap-1 sm:col-span-5">
                            <Label :for="`o-name-${gIndex}-${oIndex}`" class="text-xs">Name</Label>
                            <Input :id="`o-name-${gIndex}-${oIndex}`" v-model="opt.name" required />
                            <InputError :message="errorAt(`groups.${gIndex}.options.${oIndex}.name`)" />
                        </div>
                        <div class="grid gap-1 sm:col-span-3">
                            <Label :for="`o-price-${gIndex}-${oIndex}`" class="text-xs">Price delta</Label>
                            <Input
                                ...
                                required
                            />
                            <InputError :message="errorAt(`groups.${gIndex}.options.${oIndex}.price_delta`)" />
                        </div>
        ...
        <div class="flex items-center justify-end gap-2">
            ...
            <Button type="submit" :disabled="form.processing">{{ isEdit ? 'Save changes' : 'Create template' }}</Button>
        </div>
    </form>
</template>
