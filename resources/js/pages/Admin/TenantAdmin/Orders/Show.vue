    ...
    <TenantAdminLayout :restaurant="restaurant">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-3">
            ...
            <span class="text-xs text-muted-foreground"
                >Placed {{ formatRelativeTime(order.placedAt) }}</span
            >
        </div>
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <section class="rounded-lg border border-border bg-card p-5">
                    ...
                    >
                        ...
                        <div v-if="order.deliveryAddress.street2">
                        ...
                        <div
                            v-if="order.deliveryAddress.instructions"
                ...
                <section class="rounded-lg border border-border bg-card p-5">
                    ...
                    <ul class="mt-3 divide-y divide-border">
                        <li
                            ...
                            :key="line.id"
                            class="flex items-start justify-between py-3 text-sm"
                        >
                            <div>
                                ...
                                <div
                                    v-if="line.modifierGroups.length > 0"
                                    class="mt-0.5 text-xs text-muted-foreground"
                                >
                                    <div
                                        v-for="g in line.modifierGroups"
                                        :key="g.groupName"
                                    >
                    ...
                    <dl
                        class="mt-4 space-y-1 border-t border-border pt-4 text-sm"
                    >
                        ...
                        <div
                            v-if="order.tipCents > 0"
                            class="flex justify-between text-muted-foreground"
                        >
                            <dt>Tip</dt>
                            <dd class="tabular-nums">
                                {{ formatCents(order.tipCents) }}
                            </dd>
                        </div>
                        <div
                    ...
                    <p
                        v-if="order.notes"
                        class="mt-4 rounded-md bg-muted/40 p-3 text-sm text-muted-foreground italic"
                    >
                        Notes: {{ order.notes }}
                    </p>
                </section>
            ...
            <aside class="space-y-6">
                ...
                <section class="rounded-lg border border-border bg-card p-5">
                    ...
                    <ol class="mt-4 space-y-4">
                        ...
                        >
                            <span
                                class="absolute top-1.5 left-0 inline-block size-2 rounded-full"
                                :class="
                                    statusBadgeClasses(event.toStatus)
                                        .split(' ')
                            ...
                            <div class="text-xs text-muted-foreground">
                                ...
                                <span v-if="event.userName">
                                    · by {{ event.userName }}</span
                                >
                            </div>
                            <p
                                v-if="event.note"
        ...
        <Dialog v-model:open="cancelOpen">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        We'll email the customer to let them know. You can
                ...
                <textarea
                    ...
                    class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground shadow-sm placeholder:text-muted-foreground focus:border-ring focus:ring-1 focus:ring-ring focus:outline-none"
                />
                <DialogFooter>
                    <Button
                        type="button"
